...
public class MyStorage {
    ...
    static void prod_remove(Scanner s){
        ...
        for (int i = 0; i < products.length; ++i){
            if (itemName.equals(products[i])){  // 입력한 제품명과 일치하는 제품을 찾으면
                foundIdx = i;  // 인덱스 저장
                products[i] = EMPTY;
                counts[i] = 0;
                break;  // 반복 중단
    ...
    public static void prod_amount_add(Scanner s) {
        ...
        int cnt;
        try{
            cnt = Integer.parseInt(s.nextLine());  // 수량 입력 받기
        }
    ...
    public static void prod_amount_decrease(Scanner s) {
        ...
        int foundIdx = -1;  // 찾은 인덱스 초기화
        for (int i = 0; i < products.length; ++i){
            if (output.equals(products[i])){  // 입력한 제품명과 일치하는 제품을 찾으면
                foundIdx = i;  // 인덱스 저장
                break;  // 반복 중단
            }
        }
        ...
        if(cnt > counts[0]){
            ...
            System.out.println("출고량을 초과 할 수 없습니다.");
            return;
        }
        else {
